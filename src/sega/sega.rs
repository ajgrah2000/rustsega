...
impl Sega {
    ...
    pub fn get_console_size() -> graphics::display::ConsoleSize {
        graphics::display::ConsoleSize::new(graphics::vdp::Constants::SMS_WIDTH, graphics::vdp::Constants::SMS_HEIGHT)
    }
    ...
    pub fn run_sega(me: &mut Sega) -> bool {
        ...
        if !me.draw_loop(pixel_format, &console_size, Sega::DISPLAY_UPDATES_PER_KEY_EVENT) {
            return false;
        }
        true
    }
    ...
    pub fn power_sega(&mut self) {
        ...
        let console_size = Self::get_console_size();
        let window_size = graphics::display::WindowSize::new(frame_width, frame_height, console_size, self.fullscreen);
    ...
    pub fn new(debug: bool, realtime: bool, stop_clock:clocks::ClockType, cartridge_name: &String, fullscreen: bool) -> Self {
        ...
        Self { core, debug, realtime, stop_clock, fullscreen, powered:false, sdl_context:None , canvas:None, audio_queue:None}
    }
        ...
        if self.canvas.is_some() {
            ...
            while display_refreshes < iterations {
                ...
                if 0 == audio_steps % Sega::CPU_STEPS_PER_AUDIO_UPDATE {
                    ...
                    let mut audio_queue = self.audio_queue.as_mut().expect("Optional audio not set");
                    sound::SDLUtility::top_up_audio_queue(&mut audio_queue, |fill_size| {self.core.ports.audio.get_next_audio_chunk(fill_size)});
                }
                audio_steps += 1;
    ...
    pub fn configure_sdl(&mut self, window_size: graphics::display::WindowSize, pixel_format: pixels::PixelFormatEnum) {
        ...
        match self.canvas {
            Some(ref mut v) => { v.set_logical_size(window_size.console_size.console_width as u32, window_size.console_size.console_height as u32).unwrap();
            },
            None => {}
        }
